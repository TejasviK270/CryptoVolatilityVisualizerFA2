...
# ─────────────────────────────────────────────────────────────────────────────
# GITHUB CSV URL — Stage 7 Requirement
# ─────────────────────────────────────────────────────────────────────────────
GITHUB_CSV_URL = (
...
# ─────────────────────────────────────────────────────────────────────────────
# HELPERS — Stage 4 Data Preparation
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# STAGE 4: UPDATED 30-DAY DATE FILTER
# ─────────────────────────────────────────────────────────────────────────────
def date_range_filter(df, key_prefix="main"):
    ...
    if total_days_in_data < 1: 
        return df

    st.sidebar.subheader("📅 Date Range Filter")
    
    # Range is now updated to 30 days
    days_back = st.sidebar.slider(
        "Show last N days",
        min_value=1,
        max_value=min(max(total_days_in_data, 1), 30),
        value=min(30, total_days_in_data),
        key=f"{key_prefix}_slider"
    )
    
    start_date = max_date - timedelta(days=days_back)
    st.sidebar.info(f"📆 Showing data from: {start_date} to {max_date}")
...
if mode == "📊 Real Bitcoin Data":
    ...
    # Stage 5: Line Graph of Price Over Time
    st.subheader("📈 Bitcoin Price Over Time")
    fig1 = px.line(df, x="Timestamp", y="Price", labels={"Price": "Price (USD)"})
    st.plotly_chart(fig1, use_container_width=True)

    # Stage 5: High vs Low Comparison
    ...
    # Stage 6: Sidebar Controls
    pattern = st.sidebar.selectbox("Pattern:", ["Sine Wave (Smooth Cycles)", "Sine + Noise", "Random Noise"])
    amp = st.sidebar.slider("Amplitude (Swing Size)", 100, 5000, 1000)
    freq = st.sidebar.slider("Frequency (Swing Speed)", 0.1, 5.0, 1.0)
    drift = st.sidebar.slider("Drift (Long-term Slope)", -50, 50, 10)
    noise = st.sidebar.slider("Noise Level", 0, 500, 100)
    num_days = st.sidebar.slider("Simulation Days", 1, 30, 7)
...
st.markdown("---")
st.markdown("**FinTechLab Pvt. Ltd.** | Mathematics for AI-II | FA-2: Dashboarding and Deployment [cite: 1, 2]")
